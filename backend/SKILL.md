...
### Dispatch cancellation cleanup
When a dispatch is cancelled (any of the 3 paths: driver reject, dashboard cancel, dashboard delete), its `container_events` and `driver_positions` are deleted. This prevents stale milestone markers and GPS breadcrumbs from cluttering the map. Completed dispatches keep all data for audit trail and detention evidence. The `dispatch_cancelled` event itself is preserved (logged AFTER the cleanup).
