...
26. **Lingering timestamps after dispatch cancel.** When a dispatch is cancelled, container milestone timestamps (`pod_full_out_at`, `empty_terminated_at`) remain set from the old dispatch. Frontend shows stale dates even though no active dispatch exists. The map also shows the old GPS trail because `container_events` from the cancelled dispatch still have lat/lng data. Root cause: cancel endpoint only updated `dispatches.status` without resetting the container. Fix: cancel dispatch endpoint must also: (a) Reset container: `pod_full_out_at=NULL`, `empty_terminated_at=NULL`, `ui_status` back to pre-dispatch state (AT_PORT if discharged), `available_for_pickup=true`. (b) Log `dispatch_cancelled` event to `container_events`. (c) The GPS trail on the map should filter by active dispatch only — not show events from cancelled dispatches.
