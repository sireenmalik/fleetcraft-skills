...
**Why `requireDispatcher` is permissive** — the dispatcher dashboard at `myfleetcraft.com` still makes API calls without an Authorization header. Full dispatcher auth (login endpoint + frontend JWT plumbing) is Spec 0020, currently **deferred** and bundled with the multi-tenant SaaS spec — multi-tenant needs `org_id` from a verified token anyway, so we're doing the JWT plumbing once instead of twice. This middleware closes only the cross-role replay vector from Spec 0016 §9 (customer JWT pasted into a dispatcher call). When dispatcher login lands, flip the `if (!header) return next()` branch to `return res.status(401).json({ error: 'No token' })` and test the dashboard.

**Interim network gate — do this BEFORE onboarding customer #2.** While `requireDispatcher` is permissive, any caller who knows the URL can hit dispatcher endpoints. For single-tenant this is fine (DNS is a small barrier and nobody else uses the dashboard yet). Before a second customer joins, add an nginx IP allowlist on `api.myfleetcraft.com` for the dispatcher routes — 15-min config change, zero code, zero SPOFs, rollback in seconds. If you're reading this and already onboarding — stop and gate first.
...
23. **Dead ui_status values.** AT_PORT, OUT_FOR_DELIVERY, EMPTY_RETURNED no longer exist. A CHECK constraint on containers.ui_status enforces the 18 Spec 0029 values. Any code writing old values will crash with a constraint violation. When adding new code that writes ui_status, only use values from the STATUS_RANK map.
24. **Dispatch cancel must set AVAILABLE.** Cancelling a dispatch sets ui_status = 'AVAILABLE', not AT_PORT. Never null out FTU timestamps (pod_full_out_at, empty_terminated_at) on cancel — they record physical events.
...
25. **Cross-phase column writes.** driver.js must NEVER write FTU-owned columns (pod_full_out_at, empty_terminated_at, available_for_pickup, pod_discharged_at, pod_arrival, pod_eta). These are carrier events owned by FTU. The driver app writes containers.ui_status (ranks 9-18 ONLY), container_events (milestone ledger), and driver_positions (GPS). That is its entire write surface on the database. No other columns on the containers table. See Spec 0029 section 17 for the full column ownership table.
